...
const emitChange = () => {
    emit(
        "periodChange",
        allDaysInMonth.value[0],
        allDaysInMonth.value[allDaysInMonth.value.length - 1],
    );
};
...
const allDaysInMonth = computed(() =>
    getAllDaysInMonth(currentMonth.value, currentYear.value),
);
...
const getDayChildren = (calendarDate: Date): string[] => {
    ...
    const calendarEventsByDays = getCalendarEventsByDays(props.calendarEvents)[
        key
    ];
    return (
...
<template>
    <section>
        ...
        >
            <CalendarCell
                v-for="calendarDay in allDaysInMonth"
